...
export async function uploadPDF({ arquivo, tipo }: UploadPDFRequest) {
    ...
    const response = await fetch(`https://${process.env.VERCEL_URL}/api/transcricoes`, {
        method: "POST",
        body: formData,
    });
...
export async function getTranscription(id: string): Promise<Transcription | null> {
    const response = await fetch(`https://${process.env.VERCEL_URL}/api/transcricoes/${id}`);
...
export async function getPDF(id: string): Promise<Uint8Array<ArrayBufferLike>> {
    const response = await fetch(`https://${process.env.VERCEL_URL}/api/transcricoes/${id}/file`, {
        headers: {
